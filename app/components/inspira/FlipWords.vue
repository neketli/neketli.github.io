...
watch(isVisible, (newValue) => {
  if (newValue) startTimeout()
})
</script>

<template>
  <div class="relative inline-block px-2">
    <Transition @after-leave="$emit('animationComplete')" @after-enter="$emit('animationStart')">
      <div
        v-show="isVisible"
        class="relative z-10 inline-block text-left text-white"
        :class="props.class"
      >
...
.v-enter-active { animation: enterWord 0.6s ease-in-out forwards; }
.v-leave-active { animation: leaveWord 0.6s ease-in-out forwards; }
@keyframes enterWord {
